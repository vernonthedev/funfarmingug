...
function MobileSubmenu({ sub, openDropdown, toggleDropdown }: { sub: { label: string; href: string; submenu?: Array<{ label: string; href: string }> }; openDropdown: string | null; toggleDropdown: (label: string) => void }) {
  if (sub.submenu) {
    return (
      <>
        <Link
        ...
        {openDropdown === sub.label && (
          <ul className="sub-menu menu1 mobile-submenu-nested">
            {sub.submenu.map((nested) => (
              <li key={nested.label}><Link href={nested.href}>{nested.label}</Link></li>
            ))}
          </ul>
        )}
      </>
...
function DesktopSubmenu({ sub }: { sub: { label: string; href: string; submenu?: Array<{ label: string; href: string }> } }) {
  if (sub.submenu) {
    return (
      <>
        <Link href={sub.href} className="span-arrow" onClick={(e) => e.preventDefault()}>
          {sub.label}
          <span><i className="fa-solid fa-angle-right d-xl-inline-block d-none"></i></span>
        </Link>
  ...
  return (
    <>
      ...
      <div className={`homepage1-body ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div className="vl-offcanvas-wrapper">
            <div className="vl-offcanvas-header d-flex justify-content-between align-items-center mb-90">
              <div className="vl-offcanvas-logo">
                <Link href="/"><img src="/assets/img/logo/logo-hm6.png" alt="Orgaanic Logo" /></Link>
              </div>
              <div className="vl-offcanvas-close">
                <button className="vl-offcanvas-close-toggle" onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <div className="vl-offcanvas-menu d-xl-none mb-40">
              <nav>
                <ul className="mobile-nav">
                  {navItems.map((item) => (
                    <li key={item.label} className={item.dropdown.length > 0 ? 'has-dropdown' : ''}>
                      <Link
                        href={item.href}
                        className={item.dropdown.length > 0 ? 'dropdown-toggle' : ''}
                        onClick={(e) => {
                      ...
                      >
                        ...
                        {item.dropdown.length > 0 && (
                          <span>
                            <i className={`fa-solid fa-angle-down ${openDropdown === item.label ? 'rotate-180' : ''}`}></i>
                          </span>
                        )}
                      </Link>
                      {item.dropdown.length > 0 && openDropdown === item.label && (
                        <ul className="sub-menu mobile-submenu">
                          {item.dropdown.map((sub) => (
                            <li key={sub.label}>
                              <MobileSubmenu sub={sub} openDropdown={openDropdown} toggleDropdown={toggleDropdown} />
                            </li>
                          ))}
                        </ul>
                      )}
            ...
            <div className="vl-offcanvas-info">
              ...
              <div className="space20"></div>
              <span><a href="mailto:cattlefarm@orgaanic.com"><i className="fa-regular fa-envelope"></i> cattlefarm@orgaanic.com</a></span>
              <span><a href="tel:+8801712345678"><i className="fa-solid fa-phone"></i> +880 1712345678</a></span>
              <span><a href="#"><i className="fa-solid fa-location-dot"></i> Charabari, Shingra, Natore, Rajshahi, Bangladesh</a></span>
            </div>
            <div className="space20"></div>
        ...
        <div className="vl-offcanvas-overlay" onClick={() => setIsMobileMenuOpen(false)}></div>
      </div>

      {/* Main Header */}
      <header className={`homepage6-body ${isScrolled ? 'scrolled' : ''}`}>
        <div className={`vl-header-area vl-transparent-header ${isScrolled ? 'sticky' : ''}`} id="vl-header-sticky">
          {/* Top Header Area */}
                        ...
                        {navItems.map((item) => (
                          <li key={item.label} className={item.dropdown.length > 0 ? 'has-dropdown' : ''}>
                            <Link
                              ...
                              className={item.dropdown.length > 0 ? 'dropdown-toggle' : ''}
                              onClick={(e) => {
                                if (item.dropdown.length > 0 && item.href === '#') {
                                  e.preventDefault();
                                }
                              }}
                            >
                              {item.label}
                              {item.dropdown.length > 0 && <span><i className="fa-solid fa-angle-down d-xl-inline d-none"></i></span>}
                            </Link>
                            {item.dropdown.length > 0 && (
                              <ul className="sub-menu">
                                {item.dropdown.map((sub) => (
                                  <li key={sub.label}>
                                    <DesktopSubmenu sub={sub} />
                                  </li>
                                ))}
                              </ul>
                            )}
                ...
                <div className="col-xl-3 col-md-6 col-6">
                  <div className="vl-menu-sidebar-area">
                    <div className="sidebar-cart sidebar-cart6">
                      <img src="/assets/img/icon/cart-icon-hm6.svg" alt="Cart" />
                      <span><a href="#">0</a></span>
                    </div>
                    <div className="menu-line">
                      <svg xmlns="http://www.w3.org/2000/svg" width="1" height="25" viewBox="0 0 1 25" fill="none">
                        <path d="M0.5 0.5L0.499999 24.5" stroke="#1A5632" strokeOpacity="0.3" strokeLinecap="round" />
                      </svg>
                    </div>
                    <div className="vl-header-btn d-none d-xl-block text-end">
                      <div className="header6-btn">
                        <Link href="/contact" className="btn-home6">Start Growing Now</Link>
                      </div>
                    </div>
                    <div className="vl-header-action-item d-block d-xl-none">
                      <button type="button" className="vl-offcanvas-toggle" onClick={() => setIsMobileMenuOpen(true)}>
